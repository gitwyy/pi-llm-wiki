...
export class Runtime {
  ...
  report(summary: string, opts?: { display?: boolean; ui?: { notify: Notify } }): void {
    ...
    // still recoverable instead of vanishing without a trace. Skipped when the
    // vault root is not yet known (ensureConfig has not run): writing into
    // whatever directory the host happens to run from would pollute it.
    if (this.cwd) {
      try {
        const paths = getVaultPaths(this.cwd);
