...
// ── Search filter tests ──────────────────────────────────
describe("/wiki-model picker search filter", () => {
  let handler: Handler;
  let runtime: { config: { taskModel?: { provider: string; id: string } } };
  const dirs: string[] = [];
  let priorAgentDir: string | undefined;
  ...
  function project(tag: string) {
    const tmp = mkdtempSync(join(tmpdir(), `wiki-model-filter-${tag}-`));
    dirs.push(tmp);
    mkdirSync(join(tmp, ".pi"), { recursive: true });
    return tmp;
  }

  it("filters models by typing characters", async () => {
    const tmp = project("type");
    const h = makeCtx({
      cwd: tmp,
      models: [
  ...
  it("clears filter on first Esc, closes on second Esc", async () => {
    ...
    const screen = h.screen.current!;
    screen.handleInput("a");
    let rendered = screen.render(80).join("\n");
    expect(rendered).toContain("claude");
    expect(rendered).not.toContain("gpt-4o");
    // Esc clears filter
    screen.handleInput(ESC);
    rendered = screen.render(80).join("\n");
    expect(rendered).toContain("gpt-4o");
    expect(rendered).toContain("claude");
    // Esc again closes picker
    screen.handleInput(ESC);
    await pending;
  });

  it("backspace removes last filter character", async () => {
    const tmp = project("backspace");
    const h = makeCtx({
      cwd: tmp,
      models: [
        { provider: "anthropic", id: "claude-haiku" },
    ...
    expect(rendered).toContain("claude");
    expect(rendered).not.toContain("gpt-4o");
    // Backspace removes the "a"
    screen.handleInput("backspace");
    rendered = screen.render(80).join("\n");
    expect(rendered).toContain("gpt-4o");
    expect(rendered).toContain("claude");
    screen.handleInput(ESC);
